...
async def health():
    ...
    return HealthResponse(
        ...
        gpu_free_mb=None,  # GPU managed by Ollama
        version="5.0.0",
    )
...
async def chat_ws(ws: WebSocket):
    ...
    # Active-turn cancellation registry. The receiver sets these when a
    # {"type":"cancel"} frame arrives so the producer thread aborts the
    # in-flight LLM generation immediately (within one chunk).
    active = {
        "turn_id": None,        # str | None — currently-streaming turn
        "cancel":  None,        # threading.Event | None
    }

    async def _receiver():
        """Read query/cancel frames from the gateway and act on them."""
        try:
            async for raw in ws.iter_text():
                ...
                if ftype == "cancel":
                    # Hard barge-in: abort the active LLM generation and drop
                    # any queries already enqueued behind it. The gateway is
                    # the single source of truth for what to do next.
                    target_tid = frame.get("turn_id")
                    cur_tid    = active.get("turn_id")
                    cur_evt    = active.get("cancel")
                    if cur_evt is not None and (target_tid is None or target_tid == cur_tid):
                        log.info(f"[ws:{conn_id}] cancel received for turn:{cur_tid}")
                        cur_evt.set()
                    # Drain queued queries — they belong to the cancelled session
                    drained = 0
                    while not query_q.empty():
                        try:
                            ...
                            break
                    if drained:
                        log.info(f"[ws:{conn_id}] cancel drained {drained} pending queries")
                    # Clear dedup cache so the next barge-in query is never
                    # suppressed as a duplicate of the cancelled turn
                    dedup.clear()
                    continue
    ...
    async def _processor():
        ...
        while True:
            ...
            # Publish this turn so the receiver's cancel handler can target it.
            active["turn_id"] = turn_id
            active["cancel"]  = cancel_event
            ...
            try:
                ...
                # Clear active-turn registration regardless of outcome.
                if active.get("turn_id") == turn_id:
                    active["turn_id"] = None
                    active["cancel"]  = None
