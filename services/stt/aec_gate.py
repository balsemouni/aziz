...
class AECGate:
    ...
    def stop_immediate(self):
        """Stop suppression instantly with NO echo tail.
        Use when a barge-in is confirmed so the user's speech is not
        blocked by the echo-tail window.
        """
        self._ai_speaking   = False
        self._ai_stopped_at = None   # skip echo tail entirely
        logger.debug("[AEC] AI stopped (barge-in) — no echo tail")
